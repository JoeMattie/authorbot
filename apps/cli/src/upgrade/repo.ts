...
/**
 * Rewrite the pin, touching only that one value.
 *
 * A reformat of the author's package.json would be noise in a pull request
 * whose whole purpose is that the diff is reviewable, so the replacement is
 * surgical rather than a parse-and-serialize round trip.
 */
export function rewritePin(packageJsonText: string, newSpec: string): string {
  const pattern = new RegExp(`("${CLI_PACKAGE.replace("/", "\\/")}"\\s*:\\s*)"[^"]*"`);
  if (!pattern.test(packageJsonText)) {
    throw new UpgradeRepoError(`could not locate the ${CLI_PACKAGE} dependency in package.json`);
  }
  return packageJsonText.replace(pattern, `$1"${newSpec}"`);
}
