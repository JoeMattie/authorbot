...
**Slice 3B release status:** the release gate has been satisfied. The v0.1.34
slice 3A dual-read Worker was deployed and verified healthy before
`0013_phase11_capabilities_backfill.sql` was packaged for v0.1.35. Migration
0013 did not ride in v0.1.34: author CI would have applied it while the pre-3A
Worker could still write new legacy rows after the one-shot backfill. The
migration, its tests, and the release review must continue to preserve that
ordering.
